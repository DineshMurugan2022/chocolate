...
export default function Header({ setIsCartOpen }: { setIsCartOpen: (open: boolean) => void }) {
  ...
  const themeColors = isCelestialTheme
    ...
    : {
      ...
      hover: 'hover:text-burnt-caramel',
      accent: 'text-burnt-caramel'
    };
      ...
      <div className={`max-w-[2000px] mx-auto px-4 md:px-6 lg:px-8 xl:px-14 h-16 md:h-24 lg:h-28 grid grid-cols-[1fr_auto_1fr] items-center transition-all duration-700 relative z-20 pointer-events-auto ${isCelestialTheme ? 'bg-[#030308]/40 backdrop-blur-3xl border-b border-white/5' : 'bg-white/30 backdrop-blur-md border-b border-gold-soft/5'}`}>
        ...
        <div className="flex items-center justify-start h-full">
          ...
          <nav className="hidden lg:flex items-center gap-3 xl:gap-6 2xl:gap-8 pr-4">
            {leftLinks.map((link) => (
              <div key={link.name} className={`relative group/dropdown ${(link.name === 'STORY' || link.name === 'OCCASION') ? 'hidden xl:block' : ''
        ...
        <div className="flex items-center justify-end h-full">
          <nav className="hidden lg:flex items-center gap-3 xl:gap-6 2xl:gap-8 pl-4 mr-4 xl:mr-8">
            {rightLinks.map((link) => (
              <div key={link.name} className={`relative group/dropdown ${(link.name === 'WORKSHOP') ? 'hidden xl:block' : ''
