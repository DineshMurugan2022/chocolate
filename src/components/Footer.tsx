...
export default function Footer() {
   ...
   return (
      <footer className={`relative ${isRubyTheme ? 'bg-onyx-black text-white border-t-4 border-heritage-red pt-20 pb-12' : 'bg-cocoa-deep mt-8 pt-10 pb-6'} text-ivory-warm px-6 md:px-20 overflow-visible`}>

         {/* Stochastic Chocolate Wave Animation - Hidden on Profile */}
         {!isRubyTheme && <ChocolateWave />}
         ...
         <div className="w-full max-w-[1400px] mx-auto relative z-10">
            <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 ${isRubyTheme ? 'gap-12 mb-16' : 'gap-8 mb-8'}`}>
               ...
               <div className={`flex flex-col items-center md:items-start text-center md:text-left ${isRubyTheme ? 'space-y-10' : 'space-y-8'}`}>
                  <div className="mb-6">
                     <Logo
                        variant={isRubyTheme ? 'white' : 'gold'}
                  ...
                  <p className={`${isRubyTheme ? 'font-display uppercase text-xs tracking-[0.2em] text-white/40 border-l-2 border-heritage-red pl-6 font-bold' : 'font-serif italic text-lg text-ivory-warm/50 border-l border-burnt-caramel/20 pl-6'} leading-relaxed md:pr-6 max-w-sm`}>
                     {isRubyTheme ? 'Pure British craft, harvested from the deepest estates. A narrative of architectural excellence.' : '"Respectfully harvesting from the finest botanical estates. A narrative of pure British craft."'}
                  </p>

                  <div className={`flex ${isRubyTheme ? 'gap-4' : 'gap-6'} justify-center md:justify-start`}>
                     {[
                        { icon: Instagram, label: 'IG', url: '#' },
               ...
               <div className={`flex flex-col items-center md:items-start ${isRubyTheme ? 'space-y-12' : 'space-y-10'}`}>
                  <div className="flex items-center gap-4">
                     ...
                     <span className={`font-body text-[10px] text-heritage-red font-black uppercase tracking-[0.5em]`}>{isRubyTheme ? 'INVENTORY' : 'Shop'}</span>
                  </div>
                  <div className="flex flex-col items-center md:items-start gap-6">
                     {(isRubyTheme ? ['COLLECTIONS', 'THE ATELIER', 'SEASONAL', 'SOCIETY', 'MANIFESTO'] : ['Heritage Collection', 'Artisan Workshop', 'Seasonal Harvests', 'Gifting', 'Our Story']).map((item) => (
                        <button
                           key={item}
               ...
               <div className={`flex flex-col items-center md:items-start ${isRubyTheme ? 'space-y-12' : 'space-y-10'}`}>
                  <div className="flex items-center gap-4">
                     <div className={`${isRubyTheme ? 'h-[2px] w-10' : 'h-[1px] w-8'} bg-heritage-red`} />
                     <span className={`font-body text-[10px] text-heritage-red font-black uppercase tracking-[0.5em]`}>{isRubyTheme ? 'CONTACT' : 'Connect'}</span>
                  </div>
                  <div className={`${isRubyTheme ? 'space-y-10' : 'space-y-8'} flex flex-col items-center md:items-start text-center md:text-left`}>
                     <div className="space-y-2 group flex flex-col items-center md:items-start">
                        <div className={`flex items-center gap-3 text-[10px] font-body font-black uppercase tracking-[0.3em] text-heritage-red`}>
                           <MapPin size={isRubyTheme ? 14 : 12} /> {isRubyTheme ? 'ESTATE HQ' : 'Headquarters'}
                        </div>
                     ...
                     <div className="space-y-2 group flex flex-col items-center md:items-start">
                        <div className={`flex items-center gap-3 text-[10px] font-body font-black uppercase tracking-[0.3em] text-heritage-red`}>
                           <Mail size={isRubyTheme ? 14 : 12} /> {isRubyTheme ? 'TERMINAL' : 'Email Support'}
                        </div>
                     ...
                     <div className="space-y-2 group flex flex-col items-center md:items-start">
                        <div className={`flex items-center gap-3 text-[10px] font-body font-black uppercase tracking-[0.3em] text-heritage-red`}>
                           <Phone size={isRubyTheme ? 14 : 12} /> {isRubyTheme ? 'VOICE_LINK' : 'Phone'}
                        </div>
               ...
               <div className={`flex flex-col items-center md:items-start ${isRubyTheme ? 'space-y-16' : 'space-y-16'}`}>
                  <div className={`${isRubyTheme ? 'space-y-12' : 'space-y-10'} w-full`}>
                     <div className="flex items-center gap-4">
                        <div className={`${isRubyTheme ? 'h-[2px] w-10' : 'h-[1px] w-8'} bg-heritage-red`} />
                        <span className={`font-body text-[10px] text-heritage-red font-black uppercase tracking-[0.5em]`}>{isRubyTheme ? 'TRANSMISSION' : 'Newsletter'}</span>
                     </div>
                     <div className={`relative group ${isRubyTheme ? 'p-10 bg-white/5 border-2 border-white/10' : 'p-6 rounded-[30px] border bg-white/[0.03] border-ivory-warm/[0.05]'} overflow-hidden w-full`}>
                        ...
                        <div className={`relative border-b-2 ${isRubyTheme ? 'border-white/10 group-hover:border-heritage-red' : 'border-ivory-warm/10 group-hover:border-burnt-caramel'} pb-4 flex items-center transition-colors`}>
                           <input
                              type="email"
                              placeholder={isRubyTheme ? "EMAIL_ENTRY" : "Your Email Address"}
                              className="w-full bg-transparent focus:outline-none font-body font-bold text-[10px] uppercase tracking-[0.4em] text-white placeholder:text-white/40"
                           />
            ...
            <div className={`flex flex-col md:flex-row items-center justify-between ${isRubyTheme ? 'gap-12 border-t-2 pt-12' : 'gap-10 border-t pt-10'} border-white/5`}>
               <div className={`flex flex-col md:flex-row items-center ${isRubyTheme ? 'gap-10' : 'gap-6 md:gap-10'}`}>
                  <span className="font-body text-[10px] font-black text-white/40 uppercase tracking-[0.5em] text-center md:text-left">© {year} BRITISH CHOCOLATE STORE</span>
                  <div className={`flex flex-wrap justify-center ${isRubyTheme ? 'gap-8' : 'gap-6'}`}>
                     <button onClick={() => navigate('/terms-of-service')} className="font-body text-[10px] font-black text-white/40 uppercase tracking-[0.5em] hover:text-burnt-caramel transition-colors">{isRubyTheme ? 'TERMS' : 'Terms of Service'}</button>
                     <button onClick={() => navigate('/shipping-policy')} className="font-body text-[10px] font-black text-white/40 uppercase tracking-[0.5em] hover:text-burnt-caramel transition-colors">{isRubyTheme ? 'SHIPPING' : 'Shipping Policy'}</button>
                     <button onClick={() => navigate('/refund-policy')} className="font-body text-[10px] font-black text-white/40 uppercase tracking-[0.5em] hover:text-burnt-caramel transition-colors">{isRubyTheme ? 'RETURNS' : 'Returns'}</button>
                  </div>
               </div>

               <div className={`flex flex-col md:flex-row items-center ${isRubyTheme ? 'gap-12' : 'gap-8 md:gap-10'} opacity-30 hover:opacity-100 transition-all text-white/60`}>
                  <div className="flex flex-col items-center md:items-end">
                     <span className="font-body text-[9px] font-black tracking-widest leading-none">{isRubyTheme ? 'REG_FSSAI' : 'FSSAI License'}</span>
                     <span className={`font-body ${isRubyTheme ? 'text-[11px] font-black' : 'text-[10px]'} mt-1 tracking-tighter`}>22425547000400</span>
                  </div>
                  <div className="flex flex-col items-center md:items-end">
                     <span className="font-body text-[9px] font-black tracking-widest leading-none">{isRubyTheme ? 'TAX_GSTIN' : 'GSTIN'}</span>
