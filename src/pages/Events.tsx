      ...
      >
        <div className="max-w-[1400px] mx-auto flex flex-col items-center text-center space-y-10">
          <div className="flex items-center gap-6">
            ...
            <span className="font-body text-[10px] font-black uppercase tracking-[0.6em] text-gold-soft/30">Occasion Curation</span>
          </div>

          <h1 className="text-5xl md:text-[8vw] font-display font-black leading-[0.8] tracking-tighter text-gold-soft">
            Signature <br /> <span className="italic font-light text-gold-soft/20 pr-4">Event</span> Collection
          </h1>
          ...
          <p className="max-w-xl font-serif text-xl md:text-2xl italic text-gold-soft/50 leading-relaxed border-l-2 border-gold-soft/20 pl-10">
            Tailored chocolate cascades perfect for your special occasion. From wedding truffles to birthday assortments.
          </p>
        </div>
      </motion.section>
      ...
      >
        <div className="flex items-center justify-center gap-6 min-w-max pb-4">
          ...
          <button
            ...
            className={`h-14 px-8 rounded-2xl font-body font-black text-[9px] uppercase tracking-[0.4em] transition-all border ${selectedEvent === '' ? 'bg-gold-soft text-black shadow-xl' : 'bg-black/40 border-gold-soft/10 text-gold-soft/40 hover:border-gold-soft hover:text-gold-soft'}`}
          >
            ALL_EVENTS
          </button>
        </div>
          ...
          {!search && !selectedEvent ? (
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                {eventsData.map((eData, i) => (
                   <motion.div key={eData.title} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
                      <CollectionCard 
                         title={eData.title} 
                         image={eData.image} 
                         description={eData.description} 
                         onClick={() => setSelectedEvent(eData.title)} 
                      />
                   </motion.div>
                ))}
             </div>
            ...
            <div className="py-40 flex flex-col items-center justify-center gap-10">
              ...
              <span className="font-body text-[10px] uppercase font-black tracking-[0.6em] text-gold-soft/20 italic">Extracting Event Matrix...</span>
            </div>
          ) : products.length === 0 ? (
            <div className="py-40 flex flex-col items-center justify-center gap-8 bg-black/20 rounded-[60px] border border-dashed border-gold-soft/10">
              ...
              <button onClick={() => { setSearch(''); setSelectedEvent(''); }} className="font-body text-[9px] uppercase font-black tracking-[0.4em] text-gold-soft border-b border-gold-soft pb-1 italic hover:text-white hover:border-white transition-all">Clear Selection</button>
            </div>
          ) : (
