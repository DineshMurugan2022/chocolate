...
export default function CorporateEvents() {
  ...
  return (
    <motion.div
      className="min-h-screen text-cocoa-deep selection:bg-gold-soft selection:text-white overflow-x-hidden relative pt-32 space-y-32"
      variants={stagger(0.16)}
      initial={reduceMotion ? false : 'hidden'}
      animate="show"
    >
      <SEO
        title="Corporate Gifting & Bespoke Enterprise Solutions | British Chocolate Store"
        description="Elevate your corporate relationships with British Chocolate. Premium artisanal gifting solutions for enterprises in Chennai, Coimbatore, and across India. Custom branding and bulk logistics available."
      />
      ...
      <motion.section className="pt-32 md:pt-40 pb-10 px-6 lg:px-20 text-center space-y-12" variants={fadeDown}>
        ...
        <h1 className="text-6xl md:text-[9vw] font-display font-black leading-[0.8] tracking-tighter text-cocoa-deep">
          The Corporate <br /> <span className="italic font-light text-burnt-caramel/20 uppercase tracking-widest pl-4">Registry</span>
        </h1>
      ...
      <motion.section
        className="px-6 lg:px-20 py-20 bg-gold-soft/[0.03] border-y border-gold-soft/10"
        variants={fadeUp}
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
          <div className="space-y-12">
            <div className="space-y-4">
              <span className="font-body text-[11px] text-burnt-caramel font-black uppercase tracking-[0.5em]">The Regional Standard</span>
              <h2 className="text-5xl md:text-7xl font-display italic font-black text-cocoa-deep leading-tight">Elite Gifting in <br /> Tamil Nadu</h2>
            </div>
            <p className="font-serif text-xl italic text-cocoa-deep/60 leading-relaxed border-l-2 border-gold-soft/20 pl-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {[
                ...
                <div key={item.title} className="space-y-2 group">
                  <h3 className="font-body text-[10px] text-burnt-caramel font-black uppercase tracking-[0.3em] group-hover:text-cocoa-deep transition-colors">{item.title}</h3>
                  <p className="font-serif italic text-sm text-cocoa-deep/40">{item.desc}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="relative aspect-square rounded-[60px] overflow-hidden border border-gold-soft/20 group">
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t from-parchment-shadow/40 via-transparent to-transparent" />
            <div className="absolute bottom-12 left-12 right-12 p-8 bg-white/40 backdrop-blur-xl rounded-2xl border border-gold-soft/10 text-center">
              <ShieldCheck className="mx-auto text-burnt-caramel mb-4" size={32} />
              <p className="font-serif italic text-lg text-cocoa-deep">Trusted by Fortune 500 partners across the Indian subcontinent.</p>
            </div>
          </div>
        </div>
      </motion.section>
      ...
      <motion.section
        ...
        viewport={sectionViewport}
      >
        <div className="max-w-7xl mx-auto rounded-[80px] bg-gradient-to-br from-parchment-light to-parchment-base border border-gold-soft/10 p-12 md:p-24 flex flex-col lg:flex-row items-center gap-16 overflow-hidden relative group">
          ...
          <div className="flex-1 space-y-10 relative z-10">
            <div className="space-y-4">
              <span className="font-body text-[11px] text-burnt-caramel font-black uppercase tracking-[0.5em]">The Corporate Signature</span>
              <h2 className="text-5xl md:text-7xl font-display italic font-black text-cocoa-deep"> Put Company logo  Here <br /> <span className="text-burnt-caramel not-italic">on Cocoa</span></h2>
            </div>
            <p className="font-serif text-2xl italic text-cocoa-deep/60 leading-relaxed">
          ...
          <div className="flex-1 relative w-full aspect-square md:aspect-auto md:h-[500px] rounded-[40px] border border-gold-soft/10 bg-white/40 flex items-center justify-center p-12 text-center group-hover:border-gold-soft transition-all duration-700">
            <div className="space-y-8">
              <div className="size-48 border-[3px] border-dashed border-gold-soft/20 rounded-full flex flex-col items-center justify-center p-8 mx-auto -rotate-12 group-hover:rotate-0 group-hover:border-gold-soft transition-all duration-1000">
                <span className="font-display text-2xl italic font-black text-burnt-caramel opacity-30 group-hover:opacity-100"> Put Your Company <br /> LOGO Here</span>
              </div>
              <div className="space-y-3">
                <p className="font-body text-[12px] font-black uppercase tracking-widest text-burnt-caramel">The Branding Matrix</p>
                <p className="font-serif italic text-cocoa-deep/40 italic">Visualizing your identity in cacao.</p>
              </div>
            </div>
      ...
      >
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-12">
          {[
            ...
            <div key={i} className="p-10 bg-gold-soft/[0.03] border border-gold-soft/10 rounded-[40px] space-y-6 hover:border-gold-soft transition-all">
              <div className="text-burnt-caramel">{s.icon}</div>
              <h3 className="text-2xl font-display italic font-black text-cocoa-deep">{s.title}</h3>
              <p className="font-serif italic text-cocoa-deep/40">{s.desc}</p>
            </div>
          ))}
        </div>
      </motion.section>
      ...
      {/* Contact Hub */}
      <motion.section className="px-6 lg:px-20 py-20 bg-gold-soft/[0.03] border-t border-gold-soft/10" variants={fadeUp}>
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-16 text-center">
          <div className="space-y-4">
            <Mail className="mx-auto text-burnt-caramel/40" size={24} />
            <p className="font-body text-[10px] font-black uppercase tracking-widest text-cocoa-deep/20">DIRECT_LINE</p>
            <p className="text-xl font-display italic text-cocoa-deep">concierge@britishchocolate.asia</p>
          </div>
          <div className="space-y-4">
            <Phone className="mx-auto text-burnt-caramel/40" size={24} />
            <p className="font-body text-[10px] font-black uppercase tracking-widest text-cocoa-deep/20">VOICE_FREQ</p>
            <p className="text-xl font-display italic text-cocoa-deep">+91 44 2833 0000</p>
          </div>
          <div className="space-y-4">
            <MapPin className="mx-auto text-burnt-caramel/40" size={24} />
            <p className="font-body text-[10px] font-black uppercase tracking-widest text-cocoa-deep/20">CENTRAL_STATION</p>
            <p className="text-xl font-display italic text-cocoa-deep">Chennai, Tamil Nadu</p>
          </div>
        </div>
      </motion.section>
