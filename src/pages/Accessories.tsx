      ...
      >
        <div className={`space-y-12 ${index % 2 === 1 ? 'lg:order-last' : ''}`}>
          <div className="space-y-6">
            ...
            <h2 className="text-6xl md:text-8xl font-display font-black leading-[0.85] text-cocoa-deep italic">
              {title.split(' ')[0]} <br /> <span className="text-burnt-caramel not-italic">{title.split(' ')[1]}</span>
            </h2>
          </div>
          <p className="font-serif text-2xl md:text-4xl italic text-cocoa-deep/40 leading-relaxed">
            {description}
          </p>
          <ul className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {features.map((f: string) => (
              <li key={f} className="flex items-center gap-4 text-[10px] font-body font-black uppercase tracking-widest text-cocoa-deep/50">
                <Check size={16} className="text-burnt-caramel" /> {f}
              </li>
            ))}
          </ul>
        </div>
            ...
            {hotspots.map((hs: any, i: number) => (
              ...
              >
                ...
                <div className="absolute left-full ml-4 top-1/2 -translate-y-1/2 w-48 p-4 bg-white/95 backdrop-blur-xl rounded-2xl border border-gold-soft/20 opacity-0 group-hover/hs:opacity-100 transition-all pointer-events-none transform translate-x-4 group-hover/hs:translate-x-0">
                  <p className="font-body text-[8px] font-black uppercase tracking-widest text-burnt-caramel mb-2">{hs.label}</p>
...
export default function Accessories() {
  ...
  return (
    <div className="min-h-screen bg-transparent text-cocoa-deep selection:bg-burnt-caramel selection:text-white overflow-x-hidden relative">
      <SEO
        title="Artisanal Accessories & Molecular Hardware | British Chocolate Store SHOP"
      ...
      <section className="h-screen flex flex-col items-center justify-center text-center px-6 relative">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.5 }} className="space-y-12 relative z-10 pt-32">
          ...
          <h1 className="text-5xl sm:text-6xl md:text-[10vw] font-display font-black leading-[0.8] tracking-tighter text-cocoa-deep">
            Precision <br /> <span className="italic font-light text-burnt-caramel/20 uppercase tracking-widest pl-4">Hardware</span>
          </h1>
      ...
      <section className="py-40 px-6 lg:px-20 bg-white/40">
        <div className="max-w-7xl mx-auto space-y-20">
          <div className="text-center space-y-4">
            <span className="font-body text-[11px] text-cocoa-deep/40 uppercase tracking-[0.8em]">Shop Hardware</span>
            <h2 className="text-6xl font-display italic font-black text-cocoa-deep">Our Collection</h2>
          </div>
          ...
          <div className="min-h-[400px]">
            {loading ? (
              ...
              <div className="py-20 flex flex-col items-center justify-center gap-8 bg-white/40 rounded-[60px] border border-dashed border-gold-soft/20 text-center px-10">
                <Settings size={48} className="text-cocoa-deep/5 mx-auto" />
                <p className="font-display text-2xl italic text-cocoa-deep/20">Contact our shop for current available custom instrumentation.</p>
              </div>
            ) : (
      ...
      <section className="px-6 lg:px-20 py-40 border-t border-gold-soft/10">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-24 items-center">
          <div className="space-y-12">
            <div className="space-y-4">
              <span className="font-body text-[11px] text-burnt-caramel font-black uppercase tracking-[0.5em]">The Final Ritual</span>
              <h2 className="text-6xl md:text-8xl font-display italic font-black text-cocoa-deep leading-[0.85]">Custom Shop <br /> Packaging</h2>
            </div>
            <p className="font-serif text-2xl md:text-3xl italic text-cocoa-deep/40 leading-relaxed max-w-lg">
              Every masterpiece requires a worthy frame. We provide pro-grade boxes and liners to secure your artisanal home creations.
            </p>
            <button className="px-12 py-6 bg-burnt-caramel text-white font-body text-[11px] font-black uppercase tracking-widest rounded-2xl hover:bg-cocoa-deep transition-all shadow-xl">Request Custom Portfolio</button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-8">
            {[
              ...
              <motion.div
                ...
                className="aspect-square bg-white/40 border border-gold-soft/10 rounded-[40px] flex flex-col items-center justify-center gap-6 group hover:border-burnt-caramel transition-all"
              >
                <div className="text-burnt-caramel opacity-30 group-hover:opacity-100 group-hover:scale-125 transition-all">
                  {item.icon}
                </div>
                <span className="font-body text-[9px] font-black uppercase tracking-widest text-cocoa-deep/40 group-hover:text-cocoa-deep">{item.label}</span>
              </motion.div>
            ))}
          </div>
        </div>
