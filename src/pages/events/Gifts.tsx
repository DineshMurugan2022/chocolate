...
export default function GiftsEvent() {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="relative z-10 space-y-8 max-w-5xl mx-auto pt-32 pb-20">
          <div className="flex items-center justify-center gap-6">
            ...
            <span className="font-body text-[10px] font-black uppercase tracking-[0.8em] text-gold-soft/70">The Gifting Protocol</span>
            <div className="h-[1px] w-20 bg-gold-soft/40" />
          </div>
          <h1 className="text-7xl md:text-[10vw] font-display font-black leading-[0.85] tracking-tighter">
            Artisanal<br /><span className="italic font-light text-gold-soft/30 uppercase tracking-widest pl-4 text-5xl md:text-[6vw]">Gifts</span>
          </h1>
      ...
      <motion.section className="px-6 lg:px-20 py-24" variants={fadeUp} viewport={sectionViewport}>
        <div className="max-w-6xl mx-auto space-y-16">
          <div className="text-center space-y-4">
            <span className="font-body text-[10px] text-gold-soft/50 uppercase tracking-[0.8em]">Our Process</span>
            <h2 className="text-4xl font-display italic font-black">The Art of Giving, Perfected</h2>
            <p className="max-w-lg mx-auto font-serif text-gold-soft/50 leading-relaxed">Four steps to a gift they will never forget.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {giftingSteps.map((s, i) => (
              <div key={i} className="space-y-6 relative">
                <div className="text-6xl font-display font-black text-gold-soft/10">{s.step}</div>
      ...
      {/* ── STORY + IMAGE ── */}
      <motion.section className="px-6 lg:px-24 py-24 bg-white/[0.02] border-y border-gold-soft/10 max-w-full" variants={fadeUp} viewport={sectionViewport}>
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="relative rounded-[40px] overflow-hidden aspect-square">
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-transparent to-cocoa-deep/60" />
            <div className="absolute bottom-8 left-8 right-8 p-6 bg-cocoa-deep/80 backdrop-blur-sm rounded-2xl border border-gold-soft/20">
              <p className="font-serif italic text-gold-soft/80 text-sm">"The finest gift has no price — only intention. We simply add chocolate."</p>
              <p className="text-gold-soft/40 text-xs mt-2 font-body uppercase tracking-widest">— The ChocoLux Gifting Atelier</p>
            </div>
          </div>
          <div className="space-y-8">
            <span className="font-body text-[10px] text-gold-soft/50 uppercase tracking-[0.8em]">Why It Matters</span>
            ...
            <p className="font-serif text-gold-soft/60 leading-relaxed text-lg">
              The best gifts aren't chosen randomly. They're chosen because the giver truly sees the recipient — their tastes, their personality, their joy. ChocoLux helps you translate that seeing into something edible, beautiful, and deeply personal.
            </p>
            <p className="font-serif text-gold-soft/60 leading-relaxed text-lg">
      ...
      <motion.section id="hampers" className="px-6 lg:px-20 py-24" variants={fadeUp} viewport={sectionViewport}>
        <div className="max-w-6xl mx-auto space-y-16">
          <div className="text-center space-y-4">
            <span className="font-body text-[10px] text-gold-soft/50 uppercase tracking-[0.8em]">Gifting Options</span>
            <h2 className="text-4xl font-display italic font-black">Choose Your Hamper</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {hamperTypes.map((h, i) => (
              <div key={i} className={`p-10 rounded-3xl border space-y-4 ${i === 1 ? 'border-gold-soft/40 bg-gold-soft/5' : 'border-gold-soft/15 bg-white/[0.02]'} hover:border-gold-soft/40 transition-colors`}>
                <div className="flex items-start justify-between">
                  <h3 className="text-2xl font-display italic font-black">{h.name}</h3>
                  <span className="text-gold-soft font-body font-black text-sm whitespace-nowrap ml-4">{h.price}</span>
                </div>
                <p className="font-serif text-gold-soft/55 leading-relaxed text-sm">{h.items}</p>
                <a href="/events" className="inline-block text-xs font-body uppercase tracking-widest text-gold-soft/50 hover:text-gold-soft border-b border-gold-soft/20 hover:border-gold-soft/50 transition-colors pb-0.5">
                  Enquire →
                </a>
              </div>
            ))}
        ...
        <div className="max-w-7xl mx-auto space-y-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {[
              ...
              { icon: <Globe size={28} />, title: 'International Shipping', desc: 'We ship select non-perishable products globally with temperature-controlled packaging. Loved ones abroad deserve ChocoLux too.' },
            ].map((s, i) => (
          ...
          {loading ? (
            ...
            <div className="py-20 flex flex-col items-center justify-center gap-8 bg-black/20 rounded-[60px] border border-dashed border-gold-soft/10">
              ...
              <div className="text-center space-y-3">
                <p className="font-display text-2xl italic text-gold-soft/30">The art of giving requires curation.</p>
                <p className="font-serif text-gold-soft/40 max-w-md">Every gift is built to order. Explore our full collection or contact our concierge for a personalized gifting experience.</p>
              </div>
