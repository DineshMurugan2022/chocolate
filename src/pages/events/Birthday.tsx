...
import { ShoppingBag, Zap, Star, Gift } from 'lucide-react';
    ...
    >
      ...
      <motion.section className="pt-24 pb-10 px-6 lg:px-20 text-center space-y-12" variants={fadeDown}>
        <div className="flex items-center justify-center gap-6">
           ...
           <span className="font-body text-[10px] font-black uppercase tracking-[0.8em] text-gold-soft/60">The Celebration Registry</span>
           <div className="h-[1px] w-12 bg-gold-soft/30" />
        </div>
        <h1 className="text-6xl md:text-[9vw] font-display font-black leading-[0.8] tracking-tighter">
          Birthday <br /> <span className="italic font-light text-gold-soft/20 uppercase tracking-widest pl-4">Festival</span>
        </h1>
        <p className="max-w-2xl mx-auto font-serif text-2xl md:text-3xl italic text-gold-soft/40 leading-relaxed">
          Unwrapping the magic of molecular joy for every orbit.
        </p>
      </motion.section>
        ...
        <div className="max-w-[1700px] mx-auto min-h-[400px]">
          {loading ? (
            <div className="py-20 flex flex-col items-center justify-center animate-pulse">
              <span className="font-body text-[10px] uppercase font-black tracking-[0.6em] text-gold-soft/20">Syncing Festive Matrix...</span>
            </div>
          ) : products.length === 0 ? (
            <div className="py-20 flex flex-col items-center justify-center gap-8 bg-black/20 rounded-[60px] border border-dashed border-gold-soft/10">
              ...
              <p className="font-display text-2xl italic text-gold-soft/20">Every birthday is a milestone. Explore our shop or contact us for custom party packs.</p>
            </div>
          ) : (
      ...
      <motion.section className="px-6 lg:px-20 py-20 bg-white/[0.02] border-y border-gold-soft/10" variants={fadeUp}>
         <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-12">
            {[
              { icon: <Zap size={32} />, title: 'Party Packs', desc: 'Bite-sized magic for grand celebrations.' },
              { icon: <Star size={32} />, title: 'Custom Toppings', desc: 'Personalized names and age markers.' },
              { icon: <Gift size={32} />, title: 'Surprise Delivery', desc: 'Midnight drops across the city.' }
            ].map((s, i) => (
