...
const testimonials = [
  { name: 'Priya & Arjun', event: 'Grand Reception, Chennai', quote: 'Our guests are still talking about those rose truffle return gifts six months later. ChocoLux made our wedding magical.' },
    ...
    >
      ...
      >
        ...
        <div className="relative z-10 space-y-8 max-w-5xl mx-auto pt-32 pb-20">
          <div className="flex items-center justify-center gap-6">
            ...
            <span className="font-body text-[10px] font-black uppercase tracking-[0.8em] text-gold-soft/70">The Eternal Registry</span>
            <div className="h-[1px] w-20 bg-gold-soft/40" />
          </div>
          <h1 className="text-7xl md:text-[10vw] font-display font-black leading-[0.85] tracking-tighter">
            Wedding<br /><span className="italic font-light text-gold-soft/30 uppercase tracking-widest pl-4 text-5xl md:text-[6vw]">Atelier</span>
          </h1>
          <p className="max-w-2xl mx-auto font-serif text-xl md:text-2xl italic text-gold-soft/60 leading-relaxed">
            Crafting the most sacred sweet chapter of your love story — one bespoke truffle at a time. From intimate elopements to grand 500-guest Tamil Nadu celebrations, ChocoLux transforms your wedding day into an edible masterpiece.
          </p>
          <div className="flex flex-wrap items-center justify-center gap-4 pt-4">
            <a href="#gallery" className="px-8 py-4 bg-gold-soft text-cocoa-deep font-body font-black uppercase tracking-widest text-xs hover:bg-white transition-colors rounded-full">
              Explore Gallery
            </a>
            <a href="#packages" className="px-8 py-4 border border-gold-soft/40 text-gold-soft font-body font-black uppercase tracking-widest text-xs hover:border-gold-soft transition-colors rounded-full">
              View Packages
            </a>
          </div>
        </div>
      ...
      <motion.section className="px-6 lg:px-24 py-24 space-y-20 max-w-7xl mx-auto" variants={fadeUp} viewport={sectionViewport}>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="space-y-8">
            <span className="font-body text-[10px] text-gold-soft/50 uppercase tracking-[0.8em]">Our Philosophy</span>
            ...
            <p className="font-serif text-gold-soft/60 leading-relaxed text-lg">
              At ChocoLux, we believe luxury is not a price point — it is a feeling. Our master chocolatiers in Tamil Nadu source the rarest single-origin cacao from Kerala's Idukki hills, blending it with native spices like cardamom, rose water, and aged saffron from Kashmir to create bonbons that are unmistakably, irreplaceably yours.
            </p>
            <p className="font-serif text-gold-soft/60 leading-relaxed text-lg">
          ...
          <div className="relative rounded-[40px] overflow-hidden aspect-square">
            ...
            <div className="absolute bottom-8 left-8 right-8 p-6 bg-cocoa-deep/80 backdrop-blur-sm rounded-2xl border border-gold-soft/20">
              <p className="font-serif italic text-gold-soft/80 text-sm leading-relaxed">"The most memorable moments in life deserve the most extraordinary flavors."</p>
              <p className="text-gold-soft/40 text-xs mt-2 font-body uppercase tracking-widest">— The ChocoLux Atelier</p>
            </div>
          </div>
        </div>
      </motion.section>
      ...
      <motion.section className="px-6 lg:px-20 py-20 bg-white/[0.02] border-y border-gold-soft/10" variants={fadeUp} viewport={sectionViewport}>
        <div className="max-w-7xl mx-auto space-y-16">
          <div className="text-center space-y-4">
            <span className="font-body text-[10px] text-gold-soft/50 uppercase tracking-[0.8em]">What We Offer</span>
            <h2 className="text-4xl font-display italic font-black">A Complete Bridal Chocolate Experience</h2>
          </div>
      ...
      <motion.section id="gallery" className="px-6 lg:px-20 py-24 space-y-16" variants={fadeUp} viewport={sectionViewport}>
        <div className="text-center space-y-4">
          ...
          <p className="max-w-xl mx-auto font-serif text-gold-soft/50 leading-relaxed">Each piece is handcrafted in small batches, ensuring a level of quality and detail that no industrial confectionery can match.</p>
        </div>
        ...
        <div className="max-w-[1700px] mx-auto min-h-[400px]">
          {loading ? (
            ...
            <div className="py-20 flex flex-col items-center justify-center gap-8 bg-black/20 rounded-[60px] border border-dashed border-gold-soft/10">
              <ShoppingBag size={48} className="text-gold-soft/10" />
              <div className="text-center space-y-3">
                <p className="font-display text-2xl italic text-gold-soft/30">Each wedding is beautifully unique.</p>
                <p className="font-serif text-gold-soft/40 max-w-md">Our wedding collections are often curated exclusively for each couple. Contact us to begin designing your personalized bridal chocolate registry.</p>
              </div>
      ...
      {/* ── PACKAGES ── */}
      <motion.section id="packages" className="px-6 lg:px-20 py-24 bg-white/[0.02] border-y border-gold-soft/10" variants={fadeUp} viewport={sectionViewport}>
        <div className="max-w-6xl mx-auto space-y-16">
          <div className="text-center space-y-4">
            <span className="font-body text-[10px] text-gold-soft/50 uppercase tracking-[0.8em]">Investment</span>
            <h2 className="text-4xl font-display italic font-black">Wedding Packages</h2>
            <p className="max-w-lg mx-auto font-serif text-gold-soft/50">All packages include complimentary custom flavor consultation and branded packaging.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {packages.map((pkg, i) => (
              <div key={i} className={`p-10 rounded-3xl border space-y-8 flex flex-col ${i === 1 ? 'border-gold-soft/50 bg-gold-soft/5 scale-105' : 'border-gold-soft/15 bg-white/[0.02]'}`}>
                {i === 1 && <span className="text-xs font-body uppercase tracking-widest text-gold-soft bg-gold-soft/20 px-3 py-1 rounded-full w-fit">Most Popular</span>}
                <div>
                  <h3 className="text-3xl font-display italic font-black">{pkg.name}</h3>
                  <p className="text-gold-soft/50 font-serif text-sm">{pkg.guests}</p>
                </div>
                <p className="text-4xl font-display font-black text-gold-soft">{pkg.price}</p>
                <ul className="space-y-3 flex-1">
                  {pkg.perks.map((perk, j) => (
                    <li key={j} className="flex items-start gap-3 font-serif text-sm text-gold-soft/70">
                      <CheckCircle size={16} className="text-gold-soft/50 mt-0.5 shrink-0" />
                      {perk}
                    </li>
                  ))}
                </ul>
                <a href="/events" className="block text-center px-6 py-3 border border-gold-soft/40 text-gold-soft rounded-full font-body uppercase tracking-widest text-xs hover:bg-gold-soft/10 transition-colors">
                  Enquire Now
                </a>
              </div>
            ))}
      ...
      <motion.section className="px-6 lg:px-20 py-24" variants={fadeUp} viewport={sectionViewport}>
        <div className="max-w-6xl mx-auto space-y-16">
          <div className="text-center space-y-4">
            <span className="font-body text-[10px] text-gold-soft/50 uppercase tracking-[0.8em]">Love Stories</span>
            <h2 className="text-4xl font-display italic font-black">What Our Couples Say</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {testimonials.map((t, i) => (
              <div key={i} className="p-10 rounded-3xl bg-white/[0.03] border border-gold-soft/10 space-y-6">
                <p className="font-serif italic text-gold-soft/70 leading-relaxed text-base">"{t.quote}"</p>
                <div>
                  <p className="font-display font-black text-gold-soft">{t.name}</p>
                  <p className="font-body text-xs text-gold-soft/40 uppercase tracking-widest">{t.event}</p>
                </div>
              </div>
            ))}
          </div>
